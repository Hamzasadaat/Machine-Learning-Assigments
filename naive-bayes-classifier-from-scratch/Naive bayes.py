    ...
    for key in coloumn:
        ...
        if (key != prediction_value):
            ...
            for fields in diff_values:
                ...
                attribute_no += 1
            what_data = int(input(' >> Select data, for coloumn `` %s ``,(Enter no) : ' % (coloumn[key])))
...
def get_likelihood_estimation(flag):
    calculation = []
...
def main():
    import csv
    with open(r"C:\Users\DarkReaper\Desktop\naive-bayes-using-python-master\tennis.csv") as csvfile:
        ...
        for row in reader:
            ...
            data.append(row)
            sr_no += 1
    # Q6).The dataset given above is training dataset. Split the data into train and validation.
    print('\n==============  SELECT A COLUMN TO PREDICT USING NAIVE BIAS  ============\n')
    no_2 = 1
    for key in data[0]:
        print('%d. %s' % (no_2, key))
        coloumn[no_2] = key
    ...
    for prob in prob_dict:
        for classes in class_value:
            if (prob_dict[prob][classes] == 0):
                likelihood = True
    # Q3) Calculate Posterior Probability for each class. In your case the number of classes are 2.
    print("\n===========  Posterior Probability  For Each  Data =============\n")
    if (likelihood == True):
        # Q5)Calculate the likelihood.
        print('>> Calculating likelihood \n')
        lap = True
        predicted_value = get_likelihood_estimation(lap)
    else:
    ...
    # Q7).Once the code is generated and its giving accurate answers on validation data.  test your model on these Unknown instances.
